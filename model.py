...
if __name__ == "__main__":
    ...
    if a == 1:
        ...
        df_0.to_csv(f'{lm_model}_exp_2_df_0_part_2t.csv', index=False)
        #print_full(dataset_to_generate)
        print(df_0)

    #else:
        # # Use only the first row to show that it works
        #sample_row_df = dataset_to_generate.iloc[0:6]
        #print(sample_row_df)
        ...
        df_1.to_csv(f'{lm_model}_exp_2_df_1_part_2t.csv', index=False)

    else:
    
        df_1_cont = pd.read_csv(f'{path2}/anp_thesis_project/{lm_model}_exp_2_df_1_v2.csv', delimiter=',')
        ...
        df_2.to_csv(f'{lm_model}_exp_2_df_2_v2.csv', index=False)


        print(df_2_int)
    #else:
        df_2_cont = pd.read_csv(f'{path2}/anp_thesis_project/{lm_model}_exp_2_df_2_v2.csv', delimiter=',')
        ...
        df_3.to_csv(f'{lm_model}_exp_2_df_3_v2.csv', index=False)
        
    #else:
        df_3_cont = pd.read_csv(f'{path2}/anp_thesis_project/{lm_model}_exp_2_df_3_v2.csv', delimiter=',')

        df_4 = post_processing_1(df_3_cont)


        df_4_c = df_4.copy()
        df_4_c.to_csv(f'{lm_model}_exp_2_df_4_v2.csv', index=False)
