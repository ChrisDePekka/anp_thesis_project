...
if __name__ == "__main__":
    ...
    if a == 1:
        ...
        df_0.to_csv(f'{lm_model}__df_0_t.csv', index=False)
        ...
        df_1.to_csv(f'{lm_model}__df_1_t.csv', index=False)

    #else:
    
        df_1_cont = pd.read_csv(f'{path2}/anp_thesis_project/{lm_model}__df_1_t.csv', delimiter=',')

        
        df_2_int = create_df(df_1_cont)

        df_2 = create_eval_prompts(df_1_cont)
        df_2.to_csv(f'{lm_model}__df_2_t.csv', index=False)
        df_2_cont = pd.read_csv(f'{path2}/anp_thesis_project/{lm_model}__df_2_t.csv', delimiter=',')

            #if a == 3:
        df_3 = generate_radio_scores(df_2_cont, df_2_int)
        df_3.to_csv(f'{lm_model}__df_3_t.csv', index=False)
        
    else:
        df_3_cont = pd.read_csv(f'{path2}/anp_thesis_project/{lm_model}__df_3.csv', delimiter=',')

        df_4 = post_processing_1(df_3_cont)


        df_4_c = df_4.copy()
        df_4_c.to_csv(f'{lm_model}__df_4.csv', index=False)
